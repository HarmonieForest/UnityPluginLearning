...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    public class MagicAction : UsableAction
    {
        ...
        public const string InfoKey_AllActionsCasted  = "Magic/AllActionsCasted";
        public const string InfoKey_ImpactData  = "Magic/ImpactData";

        [ActionModuleGroup(CasterIconGuid)]
        ...
        [ActionModuleGroup(StartIconGuid)]
        ...
        [ActionModuleGroup(CastEffectIconGuid)]
        ...
        [ActionModuleGroup(StopIconGuid)]
        ...
        public override void StartItemUse(Use useAbility)
        {
            ...
            if (IsDebugging) {
                DebugLogger.SetInfo(InfoKey_ImpactData, "Nothing since Start");
            }
        ...
        {
            // Here I override the Use Item, the base.UseItem is called from within the TriggerCast function.
            // Which is called within Trigger.UseItem.

            // The Use Item Starts here, reset the use data stream
            m_MagicUseDataStream.Reset();
            
            if (IsDebugging) {
                DebugLogger.SetInfo(InfoKey_CastState, "Use Item Trigger");
            }
        ...
        public virtual void OnStartCasting(MagicCastData castData)
        {
            ...
            DebugLogger.SetInfo(InfoKey_CastState, "On Start Casting");
            DebugLogger.SetInfo(InfoKey_ActionsCasted, "");
            DebugLogger.SetInfo(InfoKey_AllActionsCasted, "");
            DebugLogger.SetInfo(InfoKey_CastData, castData?.ToString());
        ...
        {
            if (IsDebugging) {
                DebugLogger.SetInfo(InfoKey_AllActionsCasted, $"Individual Cast Count: {individualCastCount} | All Casted Count: {allCastCount}");
            }
            
            Shared.Events.EventHandler.ExecuteEvent(GameObject, "OnMagicItemCast", CharacterItem);
        }
        ...
        {
            DebugLogger.SetInfo(InfoKey_CastState, "On Stop Casting");
            DebugLogger.SetInfo(InfoKey_CastData, castData?.ToString());
