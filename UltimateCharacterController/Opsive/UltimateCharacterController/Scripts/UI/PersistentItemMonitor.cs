...
namespace Opsive.UltimateCharacterController.UI
{
    ...
    public class PersistentItemMonitor : ItemMonitor
    {
        ...
        {
            UpdateActiveState(!characterItem.DominantItem && ItemIdentifierMatch(characterItem));
        }
        ...
        {
            if (m_CharacterInventory.GetItemIdentifierAmount(characterItem.ItemIdentifier) == 0) {
                return;
            }
            UpdateActiveState(!characterItem.DominantItem && ItemIdentifierMatch(characterItem));
        }
        ...
        /// </summary>
        /// <param name="characterItem">The item that may use the ItemIdentifier.</param>
        /// <returns>True if the item matches the ItemIdentifier used by the monitor.</returns>
        private bool ItemIdentifierMatch(CharacterItem characterItem)
        {
            if (characterItem.ItemIdentifier.GetItemDefinition() == m_ItemDefinition) {
                return true;
            }
        ...
        /// <param name="newAmount">The amount of ItemIdentifier to adjust.</param>
        protected override void OnAdjustItemIdentifierAmount(IItemIdentifier itemIdentifier, int previousAmount,
            int newAmount)
        {
            if (itemIdentifier.GetItemDefinition() != m_ItemDefinition) {
                return;
            ...
            m_PrimaryCount.text = newAmount.ToString();
        }
