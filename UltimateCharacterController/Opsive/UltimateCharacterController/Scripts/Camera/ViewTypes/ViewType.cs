...
namespace Opsive.UltimateCharacterController.Camera.ViewTypes
{
    ...
    public abstract class ViewType : StateObject
    {
        ...
        protected Rigidbody m_CharacterRigidbody;
        ...
        public Quaternion CharacterRotation { get => m_CharacterRigidbody.rotation * m_CharacterLocomotion.DesiredRotation; }
        public Vector3 CharacterPosition { get => m_CharacterRigidbody.position + m_CharacterLocomotion.DesiredMovement; }
        ...
        /// <summary>
        /// Updates the camera position after the character has moved. This ensures the camera is always using the most accurate location.
        /// The position may different if the character is on an object that moves within Update instead of FixedUpdate.
        /// </summary>
        ...
        {
            var lookPoint = m_CameraController.Anchor.position + m_CharacterLocomotion.DesiredMovement;
            lookPoint += (m_CameraController.AnchorOffset.x * m_Transform.right) + (m_CameraController.AnchorOffset.y * (m_CharacterRigidbody.rotation * Vector3.up)) + (m_CameraController.AnchorOffset.z * m_Transform.forward);
            return lookPoint;
        }
        ...
        protected Vector3 GetAnchorTransformPoint(Vector3 localPosition)
        {
            ...
            return MathUtility.TransformPoint(anchorPosition, m_CameraController.Anchor.rotation * m_CharacterLocomotion.DesiredRotation, localPosition);
        }
