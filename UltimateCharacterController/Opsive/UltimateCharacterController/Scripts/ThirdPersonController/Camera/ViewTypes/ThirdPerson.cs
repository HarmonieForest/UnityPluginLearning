...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera.ViewTypes
{
    ...
    public abstract class ThirdPerson : ViewType
    {
        ...
        [SerializeField] protected float m_HorizontalPivotFreedom = 0.25f;
        ...
        private Quaternion m_CharacterMovingPlatformRotationOffset = Quaternion.identity;
        private Quaternion m_MovingPlatformRotation = Quaternion.identity;
        private Quaternion m_PrevMovingPlatformRotation = Quaternion.identity;
        ...
        public override void ChangeViewType(bool activate, float pitch, float yaw, Quaternion baseCharacterRotation)
        {
            ...
            if (activate) {
                ...
                if (m_CharacterLocomotion.MovingPlatform != null) {
                    UpdateMovingPlatformRotation(m_CharacterLocomotion.MovingPlatform);
                }
        ...
        private void OnCharacterChangeMovingPlatforms(Transform movingPlatform)
        {
            if (movingPlatform != null) {
                UpdateMovingPlatformRotation(movingPlatform);
            } else {
                m_CharacterMovingPlatformRotationOffset = Quaternion.identity;
                m_MovingPlatformRotation = Quaternion.identity;
                m_PrevMovingPlatformRotation = Quaternion.identity;
            }
        }

        /// <summary>
        /// Updates the Character Platform Rotation variables.
        /// </summary>
        /// <param name="movingPlatform">The platform that the character is on top of.</param>
        private void UpdateMovingPlatformRotation(Transform movingPlatform)
        {
            m_CharacterMovingPlatformRotationOffset = m_BaseRotation * Quaternion.Inverse(movingPlatform.rotation);
            m_PrevMovingPlatformRotation = movingPlatform.rotation;
        }
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediateUpdate)
        {
            ...
            if (m_CharacterLocomotion.MovingPlatform != null) {
                if (m_CharacterLocomotion.AlignToUpDirection) {
                    m_MovingPlatformRotation = MathUtility.InverseTransformQuaternion(m_CharacterLocomotion.MovingPlatform.rotation, m_CharacterMovingPlatformRotationOffset) *
                        Quaternion.Inverse(MathUtility.InverseTransformQuaternion(m_CharacterLocomotion.MovingPlatform.rotation, m_BaseRotation *
                        Quaternion.Inverse(m_CharacterLocomotion.MovingPlatform.rotation)));
                } else {
                    var characterRotation = CharacterRotation;
                    var localRotation = MathUtility.InverseTransformDirection(m_CharacterLocomotion.MovingPlatform.eulerAngles - m_PrevMovingPlatformRotation.eulerAngles, characterRotation);
                ...
                m_BaseRotation *= m_MovingPlatformRotation;
            }

            // The camera should always stay aligned to the character's up direction.
            if (m_CharacterLocomotion.AlignToUpDirection) {
                var characterRotation = CharacterRotation;
                var localRotation = MathUtility.InverseTransformQuaternion(characterRotation, m_BaseRotation).eulerAngles;
                localRotation.x = localRotation.z = 0;
                m_BaseRotation = Quaternion.Slerp(m_BaseRotation, MathUtility.TransformQuaternion(CharacterRotation, Quaternion.Euler(localRotation)), m_SecondaryRotationSpeed);
            }
            ...
            return immediateUpdate ? rotation : Quaternion.Slerp(m_Transform.rotation, rotation, m_RotationSpeed);
        }
        ...
        public override Vector3 Move(bool immediateUpdate)
        {
            ...
            if (immediateUpdate) {
                ...
                targetPosition = Vector3.SmoothDamp(m_Transform.position + m_CharacterLocomotion.MovingPlatformMovement, lookPosition, ref m_SmoothPositionVelocity, m_PositionSmoothing);
            }
        ...
        public override Vector3 LookDirection(bool characterLookDirection)
        {
            ...
            var movingPlatformRotation = characterLookDirection ? Quaternion.Inverse(m_MovingPlatformRotation) : Quaternion.identity;
        ...
        public override Vector3 LookDirection(Vector3 lookPosition, bool characterLookDirection, int layerMask, bool includeRecoil, bool includeMovementSpread)
        {
            ...
            var crosshairsDeltaRotation = characterLookDirection ? Quaternion.identity : GetCrosshairsDeltaRotation();
            var platformRotation = characterLookDirection ? Quaternion.Inverse(m_MovingPlatformRotation) : Quaternion.identity;
