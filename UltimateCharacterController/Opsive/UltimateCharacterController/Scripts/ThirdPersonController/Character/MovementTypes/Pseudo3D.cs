...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Character.MovementTypes
{
    ...
    public class Pseudo3D : MovementType
    {
        ...
        public override float GetDeltaYawRotation(float characterHorizontalMovement, float characterForwardMovement, float cameraHorizontalMovement, float cameraVerticalMovement)
        {
            ...
            if (m_LookInMoveDirection) {
                if (characterHorizontalMovement != 0 || (m_AllowDepthMovement ? characterForwardMovement : 0) != 0) {
                    ...
                    if (m_Path != null) {
                        lookRotation = Quaternion.LookRotation(Quaternion.LookRotation(Vector3.Cross(m_Path.GetTangent(m_Transform.position, ref m_PathIndex), m_Transform.up)) * inputVector.normalized);
                    } else {
                ...
                if (m_PlayerInput.IsCursorVisible()) {
                    var characterCenter = m_Transform.position + (m_Transform.up * m_CharacterLocomotion.Height / 2);
                    Vector3 forward;
                    if (m_Path != null) {
                        forward = Vector3.Cross(m_Path.GetTangent(m_Transform.position, ref m_PathIndex), m_Transform.up);
                    } else {
                    ...
                    if (m_HitPlane.Raycast(ray, out distance)) {
                        ...
                        if (localHitPoint.magnitude > (m_LookRotateBuffer * Mathf.Max(1, m_CharacterLocomotion.Velocity.magnitude)) || m_InitialOrientation) {
                            ...
                            var rotation = Quaternion.LookRotation((m_Transform.TransformPoint(localHitPoint) - characterCenter).normalized, m_Transform.up);
                    ...
                    if (direction.sqrMagnitude > 0.1f) {
                        var rotation = Quaternion.LookRotation(direction.normalized, m_Transform.up);
                        return MathUtility.ClampInnerAngle(MathUtility.InverseTransformQuaternion(m_Transform.rotation, rotation).eulerAngles.y);
                    }
                }
            }
        ...
        public override Vector2 GetInputVector(Vector2 inputVector)
        {
            ...
            if (m_LookSource != null) {
                var localEuler = MathUtility.InverseTransformQuaternion(m_LookSource.Transform.rotation, Quaternion.LookRotation(Vector3.forward, m_Transform.up)).eulerAngles;
                localEuler.x = localEuler.z = 0;
                rotation *= MathUtility.TransformQuaternion(Quaternion.Euler(localEuler), Quaternion.LookRotation(Vector3.forward, m_Transform.up));
            }
