...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class AnimatorMonitor : StateBehavior
    {
        ...
        private void UpdateAnimatorParameters()
        {
            ...
            var yawAngle = MathUtility.ClampInnerAngle((m_CharacterLocomotion.DesiredRotation * Quaternion.Inverse(m_CharacterLocomotion.MovingPlatformRotation)).eulerAngles.y);
...
#if UNITY_EDITOR
                if (m_LogItemParameterChanges) {
                    Debug.Log($"{Time.frameCount} Changed Slot{slotID}ItemSlotStateIndex Trigger to {value} on GameObject {m_GameObject.name}.");
                }
#endif
        ...
        public void UpdateItemAbilityAnimatorParameters(bool immediateUpdate = false, bool forceChange = false)
        {
            ...
            for (int i = 0; i < m_CharacterLocomotion.ActiveItemAbilityCount; ++i) {
                for (int j = 0; j < m_ItemSlotSubstateIndex.Length; ++j) {
                    if (!m_DirtyItemStateIndexParameters[j] 
                        && (value = m_CharacterLocomotion.ActiveItemAbilities[i].GetItemStateIndex(j)) != -1) {
