...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterIK : CharacterIKBase
    {
        ...
        private Transform[] m_AbilityIKHintTarget;
        ...
        protected override void Awake()
        {
            ...
            m_AbilityIKHintTarget = new Transform[count];
        ...
        private void OnAttachLookSource(ILookSource lookSource)
        {
            ...
            if (m_LookSource != null) {
                ...
                m_LookAtPosition = m_Head.position + (m_LookAtPosition - m_Head.position).normalized + m_CharacterLocomotion.DesiredMovement;
            }
        ...
        /// <param name="hintTarget">The hint target location of the limb.</param>
        ...
        public override void SetAbilityIKTarget(IKGoal ikGoal, Transform target, Transform hintTarget, float duration)
        {
            ...
            SetAbilityIKTarget(ikGoal, target, hintTarget);
        }
        ...
        /// <param name="ikGoal">The limb affected by the target location.</param>
        /// <param name="target">The target location of the limb.</param>
        /// <param name="hintTarget">The target hint location of the limb.</param>
        private void SetAbilityIKTarget(IKGoal ikGoal, Transform target, Transform hintTarget)
        {
            m_AbilityIKTarget[(int)ikGoal] = target;
            m_AbilityIKHintTarget[(int)ikGoal + 1] = hintTarget; // The hint target will always be offset by 1 compared to the original.
        ...
        private void UpdateIKTargets()
        {
            ...
            m_IKTarget[(int)IKGoal.LeftElbow] = m_StartInterpolation[(int)IKGoal.LeftElbow] != -1 ? m_InterpolationTarget[(int)IKGoal.LeftElbow] :
                                                        (m_AbilityIKHintTarget[(int)IKGoal.LeftElbow] != null ? m_AbilityIKHintTarget[(int)IKGoal.LeftElbow] : m_LeftHandItemIKHintTarget);
            ...
            m_IKTarget[(int)IKGoal.RightElbow] = m_StartInterpolation[(int)IKGoal.RightElbow] != -1 ? m_InterpolationTarget[(int)IKGoal.RightElbow] :
                                                        (m_AbilityIKHintTarget[(int)IKGoal.RightElbow] != null ? m_AbilityIKHintTarget[(int)IKGoal.RightElbow] : m_RightHandItemIKHintTarget);
            ...
            for (int i = (int)IKGoal.LeftFoot; i < (int)IKGoal.Last; ++i) {
                if (i == (int)IKGoal.LeftKnee || i == (int)IKGoal.RightKnee) {
                    m_IKTarget[i] = m_StartInterpolation[i] != -1 ? m_InterpolationTarget[i] : m_AbilityIKHintTarget[i];
                } else {
        ...
        private void UpdateTargetInterpolations()
        {
            ...
            for (int i = 0; i < (int)IKGoal.Last; ++i) {
                if (m_StartInterpolation[i] != -1) {
                    ...
                    Transform ikTarget;
                    if (i == (int)IKGoal.LeftElbow || i == (int)IKGoal.LeftKnee || i == (int)IKGoal.RightElbow || i == (int)IKGoal.RightKnee) {
                        ikTarget = m_AbilityIKHintTarget[i];
                    } else {
                        ikTarget = m_AbilityIKTarget[i];
                    }
                    if (ikTarget == null) {
                        ...
                        m_InterpolationTarget[i].SetPositionAndRotation(Vector3.Lerp(ikPosition, ikTarget.position, time), Quaternion.Slerp(ikRotation, ikTarget.rotation, time));
                    }
                    m_InterpolateIKTargets = true;
                }
            }
