...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterLocomotion : StateBehavior
    {
        ...
        private Vector3 m_MovingPlatformRelativePosition;
        private Quaternion m_MovingPlatformRotationOffset;
        ...
        public Vector2 InputVector { get { return m_InputVector; } set { m_InputVector = value; } }
        public Vector3 DeltaRotation { get { return m_DeltaRotation; } set { m_DeltaRotation = value; } }
        public Quaternion DesiredRotation { get { return m_DesiredRotation; } set { m_DesiredRotation = value; } }
        public Vector3 DesiredMovement { get { return m_DesiredMovement; } set { m_DesiredMovement = value; } }
        ...
        public Vector3 LocalDesiredMovement
        {
            get { return m_Rigidbody.InverseTransformDirection(m_DesiredMovement); }
            set {
                if (value.y > 0 && m_Grounded) { m_ForceUngrounded = true; }
                m_DesiredMovement = m_Rigidbody.TransformDirection(value);
            }
        }
        public bool ForceStickToGround { get { return m_ForceStickToGround; } set { m_ForceStickToGround = value; } }
        public bool StickingToGround => m_StickToGround || m_ForceStickToGround;
        public Vector3 RootMotionDeltaPosition { get { return m_RootMotionDeltaPosition; } set { m_RootMotionDeltaPosition = value; } }
        ...
        public float GravityAccumulation { get { return m_GravityAccumulation; } set { m_GravityAccumulation = value; } }
        ...
        public bool AlignToUpDirection { get => m_AlignToUpDirection; set => m_AlignToUpDirection = value; }
        ...
        protected virtual void AwakeInternal()
        {
            ...
            // Perform an initial grounded check.
        ...
        {
            SimulationManager.RegisterCharacter(this);
        ...
        private bool CanCauseRotationCollision(Collider rotationCollider)
        {
            ...
            if (rotationCollider is CapsuleCollider) {
                ...
                direction = m_Rigidbody.InverseTransformDirection(colliderTransform.TransformPoint(capsuleCollider.center) - m_Transform.position);
                ...
                direction = m_Rigidbody.InverseTransformDirection(rotationCollider.transform.TransformPoint((rotationCollider as SphereCollider).center) - m_Transform.position);
            } else {
        ...
        public void Move(float horizontalMovement, float forwardMovement, float deltaYawRotation)
        {
        ...
        private void UpdateMovingPlatformMovement()
        {
            ...
            if (m_AlignToUpDirection) {
                var platformRotation = m_MovingPlatform.rotation;
                m_MovingPlatformRotation = MathUtility.InverseTransformQuaternion(platformRotation, m_MovingPlatformRotationOffset) *
                                    Quaternion.Inverse(MathUtility.InverseTransformQuaternion(platformRotation, m_Rigidbody.rotation * Quaternion.Inverse(platformRotation)));
            } else {
                var transformRotation = m_Rigidbody.rotation;
                var localRotation = MathUtility.InverseTransformDirection(m_MovingPlatform.eulerAngles - m_PrevMovingPlatformRotation.eulerAngles, transformRotation);
                localRotation.x = localRotation.z = 0;
                m_MovingPlatformRotation = Quaternion.Euler(MathUtility.TransformDirection(localRotation, transformRotation));
            }
            ...
            var prevMovingPlatformMovement = m_MovingPlatformMovement;
            ...
            if (!m_StickToMovingPlatform) {
                var prevMovingPlatformVelocity = prevMovingPlatformMovement / (m_TimeScale * Time.deltaTime);
        ...
        {
            if (m_InstantRigidbodyMove) {
                m_Rigidbody.rotation *= m_DesiredRotation;
            } else {
                m_Rigidbody.MoveRotation(m_Rigidbody.rotation * m_DesiredRotation.normalized);
            }

            if (m_MovingPlatform != null) {
                m_MovingPlatformRotationOffset = (m_Rigidbody.rotation * m_DesiredRotation.normalized) * Quaternion.Inverse(m_MovingPlatform.rotation);
                m_PrevMovingPlatformRotation = m_MovingPlatform.rotation;
            }
        ...
        protected virtual void UpdatePosition()
        {
            ...
            if (UsingRootMotionPosition) {
                ...
                m_MotorThrottle = m_Rigidbody.TransformDirection(localDeltaPosition) / Time.fixedDeltaTime;
            } else {
                // Dampen motor forces.
                ...
                var acceleration = m_SlopeFactor * backwardsMultiplier * m_MotorAcceleration;
        ...
        {
            m_DesiredMovement += (m_MotorThrottle + m_ExternalForce) * m_TimeScale * Time.timeScale * Time.fixedDeltaTime;

            if (!m_Grounded && UsingGravity) {
                m_GravityAccumulation += m_GravityAmount * (m_TimeScale * Time.fixedDeltaTime) / Time.timeScale;
                m_DesiredMovement += m_GravityDirection * m_GravityAccumulation * (float)Mathf.Sqrt(m_TimeScale);
            }
        }
                        ...
                        if (closestRaycastHit.distance < movementMagnitude && Vector3.Dot(closestRaycastHit.normal, -m_Up) > 0.995) {
                            ...
                            if (m_WallBounceModifier > 0) {
                                ...
                                if (bouncinessValue > 0.0f) {
                                    var magnitude = m_DesiredMovement.magnitude * bouncinessValue * m_WallBounceModifier / Time.fixedDeltaTime;
        ...
        private void DetectGround(bool sendGroundedEvents = true)
        {
                    ...
                    if (grounded) {
                        ...
                        if (UsingVerticalCollisionDetection) {
                            ...
                            if (bouncinessValue > 0) {
                                var magnitude = desiredMagnitude * bouncinessValue * m_GroundBounceModifier / Time.fixedDeltaTime;
                            ...
                            m_DesiredMovement = m_Rigidbody.TransformDirection(localDesiredMovement) + m_MovingPlatformMovement;
            ...
            )
        {
            ...
            for (int i = 0; i < m_ColliderCount; ++i) {
                ...
                int localHitCount;
                var positionOffset = MathUtility.InverseTransformPoint(m_Rigidbody.position, m_Rigidbody.rotation, m_Colliders[i].transform.position);
                // Determine if the collider would intersect any objects.
                if (m_Colliders[i] is CapsuleCollider) {
                    var capsuleCollider = m_Colliders[i] as CapsuleCollider;
                    MathUtility.CapsuleColliderEndCaps(capsuleCollider, position + positionOffset, capsuleCollider.transform.rotation, out var capsuleEndCap1, out var capsuleEndCap2);
                    ...
                    localHitCount = Physics.SphereCastNonAlloc(position + sphereCollider.transform.TransformDirection(sphereCollider.center) + positionOffset, radius, direction,
                    ...
                    var boxCollider = m_Colliders[i] as BoxCollider;
                    var extents = (MathUtility.ColliderScaleMultiplier(boxCollider) - c_ColliderSpacing) * boxCollider.size / 2;
                    localHitCount = Physics.BoxCastNonAlloc(position + boxCollider.transform.TransformDirection(boxCollider.center) + positionOffset, extents, direction,
                                                                    m_CastResults, boxCollider.transform.rotation, distance, m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
#if UNITY_EDITOR
                    if (drawDebugLine) {
        ...
        {
            // Set the transform directly instead of the Rigidbody. This will allow multiple characters to collide.
            if (m_InstantRigidbodyMove) {
                m_Rigidbody.position += m_DesiredMovement;
            } else {
                m_Rigidbody.MovePosition(m_Rigidbody.position + m_DesiredMovement);
            }

            if (m_MovingPlatform != null) {
                m_MovingPlatformRelativePosition = m_MovingPlatform.InverseTransformPoint(m_Rigidbody.position + (m_DesiredMovement));
            }
            m_Velocity = (m_DesiredMovement - m_MovingPlatformMovement) / Time.fixedDeltaTime;
        ...
        public virtual bool SetMovingPlatform(Transform movingPlatform, bool platformOverride = true)
        {
            ...
            if (m_MovingPlatform != null) {
                ...
                m_MovingPlatformRotationOffset = m_Rigidbody.rotation * Quaternion.Inverse(m_MovingPlatform.rotation);
                m_PrevMovingPlatformRotation = m_MovingPlatform.rotation;
            } else {
        ...
        {
            SimulationManager.UnregisterCharacter(this);
        }
    }
}